<script>
	...
	function correct_walk() {
		...
		const duration = flip ? make_duration($pos, from, 10) : make_duration($pos, to, 10);
...
<div class="one" style="transform: translateX({$pos}px) rotateY({flip ? 180 : 0}deg);">
	<img
		alt="a penguing walking"
		src="https://mdsvex.pngwn.io/penguin{walk ? '' : '_static'}.gif"
